...
export function FeaturesSection() {
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="absolute -top-20 left-1/2 -translate-x-1/2 w-225 h-120 pointer-events-none -z-10"
        style={{
          background:
      ...
      <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        {FEATURES.map((f, i) => (
          <motion.div
            ...
            className={`group relative rounded-[22px] border border-(--border) shadow-card hover:shadow-hover transition-all duration-300 overflow-hidden ${f.span || ""} bg-linear-to-br from-(--surface-2)/60 to-(--surface)`}
          >
            ...
            <div
              aria-hidden
              className="absolute -top-24 -right-24 w-65 h-65 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"
              style={{
                background:
            ...
            <div className="relative p-5 sm:p-6">
              <div className="flex items-start gap-3">
                <div className="h-10 w-10 rounded-2xl flex items-center justify-center shrink-0 group-hover:scale-[1.06] transition-transform duration-300 bg-(--accent-soft) text-(--accent-strong)">
...
export function SectionHeader({ eyebrow, title, sub, center = true }) {
  return (
    <div className={center ? "text-center max-w-3xl mx-auto" : "max-w-3xl"}>
      {eyebrow && (
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-(--accent-soft) text-(--accent-strong) text-[11px] font-semibold uppercase tracking-[0.12em]">
          {eyebrow}
        </div>
      )}
      <h2 className="font-display text-[34px] sm:text-[44px] lg:text-[52px] leading-[1.05] tracking-tight text-(--ink) mt-4">
        {title}
      </h2>
      {sub && (
        <p className="text-[15px] sm:text-base text-(--ink-muted) mt-4 leading-relaxed">
          {sub}
        </p>
      )}
    </div>
